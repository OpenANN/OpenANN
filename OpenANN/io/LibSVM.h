 ...
 * @param min_input sets the minimal numbers of feature for input matrix in
 * @return returns the number of loaded instances from the dataset
 */
int load(Eigen::MatrixXd& in, Eigen::MatrixXd& out, const char* filename, int min_inputs = 0);

/**
 * Read a libsvm-encoded dataset from any inputstream and load
 * its values into given in- and output matrices. 
 *
 * @param in input matrix with an unspecific dimension that
 *      will contain the data 
 * @param out output matrix with an unspecific dimension that
 *      will contain the data.
 * @param stream general STL datastream for getting libsvm-encoded datasets
 * @param min_input sets the minimal numbers of feature for input matrix in
 * @return returns the number of loaded instances from the dataset
 */
 ...
 * @param in matrix containing all input values (features)
 * @param out matrix containt all output values (classes, targets)
 * @param stream general STL datastream that will receive libsvm encoded dataset
 */
