 ...
 * @class DataSetView
 *
 * An index-based dataset wrapper for representing efficient dataset views on any OpenANN::DataSet instance
 * Its only operating on an index container that points to the current accessable instances from the
 * reference OpenANN::DataSet.
 *
...
public:
  ...
  /**
   * Create a new dataset view on a given OpenANN::DataSet by using an STL-like interface
   * on std::vector<int>::iterators for indices.
   *
   * @param dataset source instance that is referenced by this view
 ...
 *
 * @param groups std::vector reference that will be filled with new dataset views.
 ...
 * @param shuffling if so, the split is called on a shuffled version of the reference dataset
 */
 ...
 *
 * @param merging the destination DataSetView that will contain later all instances from the group
